...
```
mcp-gateway  →  agents-gateway-net (172.31.7.x)   ✗ NO ce-internal
...
|------------------|--------------------------------------|----------------------|
| `databases`      | Docker file secrets (`/run/secrets`) | ✓ Best practice      |
| `code-server`    | `.env` environment variables         | ✗ Visible in `docker inspect` |
| `grafana-prom`   | `.env` for SNMP credentials          | ✗ Visible in `docker inspect` |
| `acme-sh`        | `.env` for `CF_Token`                | ✗ Visible in `docker inspect` |
