...
|---|---|---|
| `docker compose up -d` of a published-image stack | [OK] | [OK] |
| Stacks with `build: .` (local Dockerfile) | [WARN] **Unreliable** -- UI sometimes silently uses the previously-tagged local image and skips rebuild. | [OK] |
| Stacks with `extends:` | [WARN] Container Manager does not always expand `extends` correctly. | [OK] |
| Stacks referencing `external: true` networks not yet created | [FAIL] Fails with "network not found". | [OK] (after `init-nas.sh` runs) |
| Secrets via `secrets:` `file:` | [OK] | [OK] |
...
Ryzen R1600 (Zen) at 2.6 GHz x 2 cores x Cool fan profile = roughly 30 W TDP envelope DSM will allow before clock-stepping the CPU down.
