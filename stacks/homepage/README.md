...
- **PUID/PGID:** Homepage image does not use linuxserver-style PUID; bind mounts rely on host ownership. Use **`scripts/fix-permissions.sh`** on the NAS so paths under `${STACK_ROOT}/homepage/` are `root:root`.
