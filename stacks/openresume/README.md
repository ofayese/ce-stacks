...
- **Container image:** compose pins **`yuihtt/open-resume@sha256:...`** (digest verified with `docker pull yuihtt/open-resume:latest` + `docker image inspect ... RepoDigests[0]` on a trusted host -- **2026-05-10**). Re-pin after upgrades using the **same** image name only; never copy a digest from `xitanggg/`, `itsnoted/`, or another namespace unless `docker manifest inspect` proves an identical manifest (it almost never does).
